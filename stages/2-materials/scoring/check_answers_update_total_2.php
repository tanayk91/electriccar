...
if ($_POST['check'] == "Check Answers!") {
  ...
  $add_material_scores_query = "SELECT `score` FROM `ecDB`.`materials_score` WHERE `userID` = '{$_SESSION['userID']}' AND `questionID` IN ('a6','a7','a8','a9','a10')";
  ...
  $correct_questions_query = "SELECT `score` FROM `ecDB`.`materials_score` WHERE `userID` = '{$_SESSION['userID']}' AND `score` = 10 AND `questionID` IN ('a6','a7','a8','a9','a10')";
  ...
  $_SESSION['material-2-correct_questions'] = $num_correct_questions;

  //get total number of incorrect questions
  $incorrect_questions_query = "SELECT `score` FROM `ecDB`.`materials_score` WHERE `userID` = '{$_SESSION['userID']}' AND `score` = 0 AND `questionID` IN ('a6','a7','a8','a9','a10')";
  ...
  $_SESSION['material-2-incorrect_questions'] = $num_incorrect_questions;

  //save the material score in a session variable
  $_SESSION['material-score-2'] = $total_material_score;
