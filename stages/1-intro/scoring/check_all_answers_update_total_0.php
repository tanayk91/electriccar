...
if ($_POST['check'] == "Check Answers!") {
  ...
  foreach ($selected_questions as $selected_question) {
    ...
    //converting question IDs to int to use with database
    $selected_question_int = (int)$selected_question;

    //saving questionID in the format used in the database
    $store_questionID = "p".$selected_question_int;

    //storing user answer in an array
    $answer = "";
    $question_name = "ques_".$selected_question_int;
  ...
  $add_pollution_scores_query = "SELECT `score` FROM `ecDB`.`pollution_score` WHERE `userID` = '{$_SESSION['userID']}' AND `questionID` IN ('p1','p2','p3','p4','p5')";
  ...
  $_SESSION['pollution-0-correct_questions'] = $num_correct_questions;
  ...
  $_SESSION['pollution-0-incorrect_questions'] = $num_incorrect_questions;

  //save the pollution score in a session variable
  $_SESSION['pollution-score-0'] = $total_pollution_score;
