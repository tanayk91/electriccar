<?php
  ...
  if ($_POST['start_challenge'] == "START") {
    ...
    $remove_old_material_scores_query = "DELETE FROM `ecDB`.`motor_score` WHERE `userID`= '{$_SESSION['userID']}'";
    mysqli_query($link, $remove_old_material_scores_query);

    //update total scores table
    $update_total_score_query = "UPDATE `ecDB`.`total_score` SET `total` = `total` - `motor_score` WHERE `userID`= '{$_SESSION['userID']}'";
    mysqli_query($link, $update_total_score_query);

    $reset_materials_score_query = "UPDATE `ecDB`.`total_score` SET `motor_score` = 0 WHERE `userID`= '{$_SESSION['userID']}'";
