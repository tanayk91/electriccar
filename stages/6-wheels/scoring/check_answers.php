...
if ($_POST['check'] == "Next Question") {
  ...
  while ($solution = mysqli_fetch_object($correct_answer_result)) {
    if ($answer == $solution->answerID) {
      $update_battery_score_query = "INSERT INTO `ecDB`.`wheels_score` (`userID`,`questionID`,`correct`,`time_taken`,`score`) VALUES ('{$_SESSION['userID']}','".$store_questionID."',1,10,10)";
      mysqli_query($link, $update_battery_score_query);
    } else {
      $update_battery_score_query = "INSERT INTO `ecDB`.`wheels_score` (`userID`,`questionID`,`correct`,`time_taken`,`score`) VALUES ('{$_SESSION['userID']}','".$store_questionID."',0,10,0)";
      mysqli_query($link, $update_battery_score_query);
    }
  }
}
