...
if ($_POST['check'] == "Check Answers!") {
  ...
  $store_questionID = "e".$selected_question;
  ...
  $correct_questions_query = "SELECT `score` FROM `ecDB`.`wheels_score` WHERE `userID` = '{$_SESSION['userID']}' AND `score` = 10";
  ...
  $incorrect_questions_query = "SELECT `score` FROM `ecDB`.`wheels_score` WHERE `userID` = '{$_SESSION['userID']}' AND `score` = 0";
  ...
  while ($solution = mysqli_fetch_object($correct_answer_result)) {
    if ($answer == $solution->answerID) {
      $update_wheels_score_query = "INSERT INTO `ecDB`.`wheels_score` (`userID`,`questionID`,`correct`,`time_taken`,`score`) VALUES ('{$_SESSION['userID']}','".$store_questionID."',1,10,10)";
      mysqli_query($link, $update_wheels_score_query);
    } else {
      $update_wheels_score_query = "INSERT INTO `ecDB`.`wheels_score` (`userID`,`questionID`,`correct`,`time_taken`,`score`) VALUES ('{$_SESSION['userID']}','".$store_questionID."',0,10,0)";
      mysqli_query($link, $update_wheels_score_query);
    }
  }

  $wheels_score = array();
  $total_wheels_score = 0;

  //add all the scores in the wheels section
  $add_wheels_scores_query = "SELECT `score` FROM `ecDB`.`wheels_score` WHERE `userID` = '{$_SESSION['userID']}'";
  $add_wheels_scores_result = mysqli_query($link, $add_wheels_scores_query);

  while ($wheels_single_score = mysqli_fetch_object($add_wheels_scores_result)) {
    $wheels_score[] = $wheels_single_score;
  }

  foreach ($wheels_score as $wheels_single_score) {
    $total_wheels_score = $total_wheels_score + $wheels_single_score->score;
  }
  ...
  $_SESSION['wheels-correct_questions'] = $num_correct_questions;
  ...
  $_SESSION['wheels-incorrect_questions'] = $num_incorrect_questions;

  //save the wheels score in a session variable
  $_SESSION['wheels-score'] = $total_wheels_score;

  //save the total score in a session variable
  $_SESSION['score'] = $_SESSION['score'] + $total_wheels_score;

  //update total scores table with the updated score
  $update_scores_query = "UPDATE `ecDB`.`total_score` SET `total`= `total` + '".$total_wheels_score."', `wheels_score`= '".$total_wheels_score."' WHERE `userID`= '{$_SESSION['userID']}'";
